...
- 字体设置已拆成主界面/阅读器两组，并继续细分为拉丁与 CJK 字体：`interface_latin_font_family`、`interface_cjk_font_family`、`reader_latin_font_family`、`reader_cjk_font_family`。旧 `font_family` 字段继续保留并同步为阅读器字体，用于旧库和旧备份兼容。系统字体通过 Tauri 命令 `list_system_fonts` 读取。
- Markdown 阅读器支持 fenced `mermaid` 代码块，前端会懒加载 Mermaid 并按当前主题渲染 SVG；搜索和批注的正文偏移会跳过 `.mermaid-figure`，不要把 Mermaid SVG 文本重新纳入正文锚点计算。
- Markdown fenced 代码块通过 highlight.js 手动注册常用语言高亮；相对路径图片会用章节文件所在目录解析并转换为 Tauri asset URL。点击 Markdown 图片会打开图片预览模态框，预览层接管滚轮缩放，不能让滚轮穿透到底层阅读滚动。
- 本地图片显示依赖 `src-tauri/tauri.conf.json` 中的 `app.security.assetProtocol.enable = true`、asset scope，以及 `src-tauri/Cargo.toml` 中 Tauri 的 `protocol-asset` feature；不要删除这些配置，否则 `convertFileSrc` 生成的 `asset://`/`http://asset.localhost` 图片地址会无法加载。
- 首页设置中有“划动批注”开关，持久化字段是 `slide_annotate` / `slideAnnotate`。开启后左键选中文字会直接显示“添加批注”；右键仍可显示同一个菜单。选区菜单关闭动画用 `selectionMenuCloseTokenRef` 防止旧关闭回调清掉新右键菜单，不要退回简单 `setTimeout(() => setContextMenu(null))`。
- 首页设置底部展示开源信息，点击通过 Tauri 命令 `open_project_repository` 打开系统浏览器，目标地址固定为 `https://github.com/coderyjc/AuroraMD`。普通 `<a target="_blank">` 在桌面 WebView 中不可靠。
- 首页设置已改为左侧分类、右侧内容的分栏模态框，分类包括外观、功能、快捷键、Prompt 预设、数据备份和关于。外观内包含默认首页视图、首页分页数量、表格列显示、主题系列/子主题和随机主题。
- 首页书籍区支持 Gallery 与表格视图，默认视图持久化为 `home_default_view` / `homeDefaultView`；表格列配置持久化为 `home_table_columns` / `homeTableColumns`；首页分页数量持久化为 `home_page_size` / `homePageSize`，可选 10/20/30/40。分页组件在数量未超过分页阈值时不显示。
- 阅读器设置里有“高亮变更”开关，与聚焦模式同级。开启后只高亮当前章节相较上一版本新增或修改的内容，首个版本没有上一版本时不显示变化。
- 主页和普通模态框通过全局 `user-select: none` 禁止拖选界面文字；阅读器正文、输入框、文本域、可编辑区域和导出内容恢复 `user-select: text`，不要破坏批注选区和复制。
- 首页 gallery 书籍置顶采用左侧渐变色条；对应样式是 `.book-entry::before` 和 `.book-entry.is-pinned::before`。
...
- 在首页 gallery 中以书籍卡片管理本地 Markdown 书稿；支持把 `.md` 文件或文件夹拖入书架区域，或点击上传入口选择 Markdown 文件/文件夹。
- 首页表格视图以一行一本书展示，支持行号、标题、章节数量、批注数量、上传时间和上次打开时间；用户可以拖拽表头调节列宽、点击表头排序、右键打开与 gallery 相同的菜单，并多选书籍批量删除。
- 首页 Gallery、表格视图和批注工作台支持分页，每页数量由设置中的 `homePageSize` 控制，可选 10/20/30/40，项目数量不足时隐藏分页组件。
...
- 阅读 Markdown 内容，支持代码语法高亮、相对路径图片在 Tauri 桌面环境中显示，以及点击图片预览和滚轮缩放。
- 支持 `mermaid` 代码块渲染为主题适配的 SVG 图表。
- 自动提取章节标题和大纲，支持章节列表和大纲跳转。
- 选中文本后创建跨行高亮批注，保存渲染文本锚点、上下文、标题路径、颜色和评论；默认右键弹出添加入口，开启“划动批注”后左键拖选会直接浮现添加入口。
- 阅读器左栏章节列表支持右键菜单：更新当前章节快照、在资源管理器打开源文件、删除章节索引；删除前需要二次确认，且不删除磁盘上的 Markdown 源文件。
...
- 阅读器设置包含聚焦模式、高亮变更、字体、字号、行距、正文宽度、页边距、段落间距和边框样式；主题系列和主题皮肤只保留在首页设置与 `Ctrl+K` 命令面板中。
- 聚焦模式开启后，鼠标悬浮正文元素时仅当前元素及相邻元素正常显示，其余上下文淡化；悬浮正文空白处时整体淡化。
- 首页设置采用左侧分类、右侧内容布局，支持主题系列/主题皮肤选择、随机主题、首页默认视图、首页分页数量、表格列显示、主界面/阅读器字体、划动批注、快捷键录制、Prompt 预设、本地备份和恢复，关于页展示版本更新信息和开源入口。
- 首页设置支持分别设置主界面和阅读器的中文/英文字体，字体来源于系统字体库并支持搜索选择；阅读器设置中只能修改阅读器字体。
...
- 批注、高亮、评论和上下文
- 阅读进度
- 阅读器排版设置、主界面/阅读器中英文字体、首页主题系列/主题皮肤、首页默认视图、首页表格列、首页分页数量、快捷键、聚焦模式、变更高亮和划动批注开关
...
- 最近一次系统字体读取、主界面/阅读器字体拆分和字体搜索选择更新后已通过：
  - `npm.cmd run build`
  - `npm.cmd run tauri -- build`
- 最近一次 Markdown 增强渲染、章节列表右键菜单、首页开源信息和划动批注更新后已通过：
  - `npm.cmd run build`
  - `npm.cmd run tauri -- build`
- Windows 安装包输出位置通常是：
...
- 设置-外观中新增默认首页视图、表格列显示、首页分页数量；表格上传入口合并为单个“上传”按钮，可选择 Markdown 文件或文件夹。
- 设置模态框改为左侧功能分类、右侧主界面的分栏布局；分类包括外观、功能、快捷键、Prompt 预设、数据备份和关于。
- 关于页展示当前版本更新信息和 GitHub 开源入口；开源链接通过 Tauri 命令打开系统浏览器。
- 主题设置新增子主题分页，每页 6 个；新增“试试手气~”按钮，可随机切换一款主题。
- 主界面字体和阅读器字体支持分别设置中文与英文字体，字体选择区域采用 2x2 布局；后端同步保存旧字体字段以兼容旧库和旧备份。
- 阅读器新增“高亮变更”开关，与聚焦模式同级；开启后只高亮相较上一版本新增或修改的内容，首个版本保持普通阅读状态。
- 修复先导入单个章节、再导入其所在文件夹时没有新建独立书籍的问题，文件夹导入会按文件夹创建书籍并导入所有选中章节。
- 修复开启划动批注后菜单闪现、右键首次菜单一闪而逝、开源信息无法跳转等交互问题。
- 优化阅读器设置中“聚焦模式 / 高亮变更”的控件尺寸与间距，避免标题换行和占位过大。
- 禁止首页和普通模态框内的界面文字被鼠标拖选，同时保留阅读器正文、输入框、文本域和导出内容可选。
