...
- 最近一次 release 构建输出为 `src-tauri/target/release/auroramd.exe` 和 `src-tauri/target/release/bundle/nsis/AuroraMD_0.5.1_x64-setup.exe`。
...
- 首页设置支持主题系列/主题皮肤选择、主界面/阅读器字体、划动批注、快捷键录制、本地备份和恢复，底部展示开源信息；主题设置使用左侧系列、右侧子主题的双栏布局。
...
src-tauri/target/release/bundle/nsis/AuroraMD_0.5.1_x64-setup.exe
```
...
- 批注、高亮、评论和上下文
- 阅读进度
- 阅读器排版设置、主界面字体、阅读器字体、首页主题系列/主题皮肤、快捷键、聚焦模式和划动批注开关
...
- 当前 v0.5.1 UI/UX 导入与管理模态框优化后已通过：
  - `npm.cmd run build`
  - `npm.cmd run tauri -- build`
- 最近一次 v0.5.1 release 构建输出：
  - `src-tauri/target/release/auroramd.exe`
  - `src-tauri/target/release/bundle/nsis/AuroraMD_0.5.1_x64-setup.exe`
- 最近一次自定义标题栏更新后已通过：
  - `npm.cmd run build`
...
- Markdown 阅读器支持 Mermaid 图表、highlight.js 语法高亮、相对路径本地图片和点击预览缩放。
- 阅读器左栏章节列表新增右键菜单：更新章节、在资源管理器打开、删除章节索引。
- 首页设置新增“划动批注”开关，开启后选中文字直接浮现添加批注入口；同时底部新增开源信息入口，点击打开 GitHub 仓库。
