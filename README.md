...
- 阅读界面：多套主题系列、纸书感排版、可调中英文字体、聚焦模式、变更高亮、全屏阅读和图片预览。
- 本地书架管理：导入 Markdown 文件或文件夹，按书籍和章节组织文档，支持 Gallery 与表格视图、拖拽导入、排序、分页和批量删除。
- 顺手的阅读体验：章节列表、大纲跳转、上一篇/下一篇、正文搜索、章节右键管理和阅读进度恢复。
- Markdown 增强渲染：支持代码语法高亮、相对路径本地图片和 `mermaid` 代码块图表。
- 轻量批注系统：跨行高亮、评论、批注工作台、状态筛选和批量整理；可通过右键添加批注，也可在设置中开启“划动批注”，选中文字后直接浮现添加入口。
- 章节版本管理：内容快照、版本别名、版本 Diff、章节拖拽排序；阅读器可高亮相较上一版本新增或修改的内容。
- 个性化设置：主题系列、子主题分页、随机主题、首页默认视图、首页分页数量、表格列显示、快捷键、Prompt 预设和数据备份。
- 本地优先存储：SQLite 数据库保存索引和设置，Markdown 原文保留在原始位置。
...
1. 打开 AuroraMD。
2. 拖入 `.md` 文件或包含 `.md` 文件的文件夹，或点击首页“上传”入口选择 Markdown 文件/文件夹。
3. 在导入窗口中勾选要加入书籍的 Markdown 文件，并修改书名。
4. 在首页使用 Gallery 或表格视图管理书籍；表格视图支持列宽调节、表头排序、右键菜单和多选删除。
5. 进入阅读器后，可以阅读、搜索、批注、切换主题、查看变更高亮或进入全屏阅读。需要更快批注时，可在“主页 - 设置 - 功能”中开启“划动批注”。
...
- 批注、高亮、评论和上下文
- 阅读进度
- 阅读器排版设置、主界面/阅读器中英文字体、主题、首页默认视图、首页表格列、首页分页数量、快捷键、聚焦模式、变更高亮和划动批注开关
