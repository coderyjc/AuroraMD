...
pub fn init_database(db_path: &Path) -> Result<Connection, rusqlite::Error> {
    ...
    ensure_column(
        ...
        "settings",
        "home_table_columns",
        "ALTER TABLE settings ADD COLUMN home_table_columns TEXT NOT NULL DEFAULT '{\"rowNumber\":true,\"rootPath\":true,\"chapterCount\":true,\"annotationCount\":true,\"createdAt\":true,\"lastOpenedAt\":true}'",
    )?;
    ensure_column(
        &conn,
        "settings",
    ...
    ensure_column(
        ...
        "settings",
        "auto_backup_enabled",
        "ALTER TABLE settings ADD COLUMN auto_backup_enabled INTEGER NOT NULL DEFAULT 0",
    )?;
    ensure_column(
        &conn,
        "settings",
        "auto_backup_interval_minutes",
        "ALTER TABLE settings ADD COLUMN auto_backup_interval_minutes INTEGER NOT NULL DEFAULT 30",
    )?;
    ensure_column(
        &conn,
        "settings",
        "auto_backup_directory",
        "ALTER TABLE settings ADD COLUMN auto_backup_directory TEXT NOT NULL DEFAULT ''",
    )?;
    ensure_column(
        &conn,
        "settings",
